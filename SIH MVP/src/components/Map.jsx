...
import osmGeoJSON from "../coordinates.json";
import {
...
if (!MAPBOX_TOKEN) {
  console.error(
    "🔑 VITE_MAPBOX_TOKEN is required. Please add it to your .env file."
  );
}
...
export default function Map({
      ...
      >
        <div className="bg-white/95 backdrop-blur-sm border border-gray-200 shadow-xl rounded-xl p-6 text-center max-w-md">
          ...
          <div className="text-gray-700 mb-4">
            Missing Mapbox token. Create a file named <code>.env</code> in the
            project root with:
            <br />
            <code>VITE_MAPBOX_TOKEN=your_mapbox_access_token_here</code>
          </div>
          <div className="text-sm text-gray-500">
            The rest of the UI will still work.
          </div>
        </div>
      </div>
    );
  ...
  // Add LineString paths and Polygon areas from local GeoJSON to the map
  const addOSMDataLayers = () => {
    if (!map.current || !osmGeoJSON) return;

    const sourceId = "osm-data";
    const lineLayerId = "osm-paths";
    const polyFillLayerId = "osm-polygons-fill";
    const polyOutlineLayerId = "osm-polygons-outline";

    try {
      // Add or update source
      if (map.current.getSource(sourceId)) {
        // @ts-ignore setData exists for GeoJSONSource
        map.current.getSource(sourceId).setData(osmGeoJSON);
      } else {
      ...
      if (!map.current.getLayer(polyFillLayerId)) {
        map.current.addLayer({
          id: polyFillLayerId,
          type: "fill",
          source: sourceId,
      ...
      if (!map.current.getLayer(polyOutlineLayerId)) {
        map.current.addLayer({
      ...
      // Interactions: show simple popup with key tags
      const makePopupHtml = (p = {}) => {
        const name = p.name || p.alt_name || p["name:en"] || "Unnamed feature";
        const type =
          p.highway ||
          p.railway ||
          p.waterway ||
        ...
        return `
          <div style="font-family: system-ui, -apple-system, Segoe UI, Roboto, sans-serif; max-width: 280px;">
            <div style="font-weight: 700; margin-bottom: 6px;">${name}</div>
            <div style="font-size: 12px; color: #374151;">Type: <strong>${type}</strong></div>
            <div style="font-size: 12px; color: #374151;">Access: <strong>${access}</strong></div>
          </div>
        `;
      };

      ["click", "mouseenter", "mouseleave"].forEach((evt) => {
        // Guard against double-binding across style reloads
        try {
          map.current.off(evt, lineLayerId, () => {});
        } catch {}
        try {
          map.current.off(evt, polyFillLayerId, () => {});
        } catch {}
        try {
          map.current.off(evt, polyOutlineLayerId, () => {});
        } catch {}
      });

      const attachHandlers = (layerId) => {
        try {
          ...
          console.warn("Popup binding failed for", layerId, e);
        }
      };

      attachHandlers(lineLayerId);
      attachHandlers(polyFillLayerId);
      attachHandlers(polyOutlineLayerId);

      // Fit map to data on first add only
      try {
        if (!map.current.__fittedOSMBounds) {
          const bounds = new mapboxgl.LngLatBounds();
          const feats = Array.isArray(osmGeoJSON.features)
            ? osmGeoJSON.features
            : [];
          feats.forEach((feat) => {
          ...
          map.current.__fittedOSMBounds = true;
        }
      } catch (e) {
        console.warn("Could not fit to OSM data bounds:", e);
      }
    } catch (e) {
      console.warn("Failed to add OSM data:", e);
    }
  };
  ...
  useEffect(() => {
    ...
    map.current.on("load", () => {
      ...
      try {
        if (
          map.current.getSource("composite") &&
      ...
      // Add OSM paths and polygons from local GeoJSON
      addOSMDataLayers();
    });
  ...
  const handleSwitchStyle = () => {
    console.log(
      `🔄 Toggling from ${isSatellite ? "satellite" : "default"} to ${
    ...
    if (map.current) {
      ...
      map.current.once("styledata", () => {
        ...
        // Re-add OSM paths/polygons after style reload
        addOSMDataLayers();
  ...
  useEffect(() => {
    ...
    if (liveLocation) {
      ...
      el.style.background =
        "radial-gradient(circle at 8px 8px, #3b82f6 70%, #fff 100%)";
  ...
  useEffect(() => {
    ...
    console.log(
      `🗺️ Adding ${geofenceData.features?.length || 0} geofences to map`
    );

    // Clean old layers/sources
    const layerIds = [
      "geofence-weather",
      "geofence-air-quality",
      "geofence-disasters",
      "geofence-traffic",
      "geofence-industrial",
      "geofence-elevation",
    ...
    const clickLayers = layerIds.filter(
      (l) =>
        l.startsWith("geofence-") &&
        !["geofence-borders", "geofence-all-debug"].includes(l)
    );
  ...
  return (
    <>
      ...
      {!isLoadingData && (
        <div className="absolute top-6 right-6 bg-white/95 backdrop-blur-sm rounded-xl p-3 shadow-lg z-10 border border-gray-200 transition-all duration-700">
          <p className="text-sm font-medium">
            Viewing:{" "}
            <span className="capitalize text-indigo-600">{activeItem}</span>
          </p>
      ...
      <div
        className={`absolute bottom-6 right-6 z-10 transition-all duration-700`}
      >
        <StyleSwitchButton
      ...
      <div
        className={`absolute top-60 right-6 bg-white/95 backdrop-blur-sm rounded-xl p-4 shadow-lg z-10 max-h-80 overflow-y-auto w-64 transition-all duration-700`}
      >
        ...
        {geofenceData?.metadata && (
          <div className="text-xs text-gray-500 mb-3 border-b pb-2">
            🔄 Updated:{" "}
            {new Date(geofenceData.metadata.generated).toLocaleTimeString()}
            <br />
        ...
        <div className="space-y-2 text-xs">
          ...
          <div className="flex items-center">
            ...
            <span>
              Weather ({geofenceData?.metadata?.breakdown?.weather || 0})
            </span>
          </div>
          <div className="flex items-center">
            ...
            <span>
              Air Quality (
              {geofenceData?.metadata?.breakdown?.additional_simulated
                ? Math.floor(
                    geofenceData.metadata.breakdown.additional_simulated * 0.2
                  )
                : 0}
              )
            </span>
          ...
          <div className="flex items-center">
            ...
            <span>
              Disasters ({geofenceData?.metadata?.breakdown?.earthquakes || 0})
            </span>
          </div>
          <div className="flex items-center">
            <div className="w-3 h-3 rounded-full bg-purple-600 mr-2"></div>
            <span>
              Traffic ({geofenceData?.metadata?.breakdown?.traffic || 0})
            </span>
          </div>
          <div className="flex items-center">
            <div className="w-3 h-3 rounded-full bg-gray-600 mr-2"></div>
            <span>
              Industrial ({geofenceData?.metadata?.breakdown?.industrial || 0})
            </span>
          </div>
          <div className="flex items-center">
            <div className="w-3 h-3 rounded-full bg-green-700 mr-2"></div>
            <span>
              Elevation ({geofenceData?.metadata?.breakdown?.elevation || 0})
            </span>
          </div>
          <div className="flex items-center">
            ...
            <span>
              Crime/Security (
              {geofenceData?.metadata?.breakdown?.crime_security || 0})
            </span>
          </div>
          <div className="flex items-center">
            <div className="w-3 h-3 rounded-full bg-blue-500 mr-2"></div>
            <span>
              Crowds ({geofenceData?.metadata?.breakdown?.crowds || 0})
            </span>
          </div>
          <div className="flex items-center">
            ...
            <span>
              ...
              {geofenceData?.metadata?.breakdown?.additional_simulated
                ? Math.floor(
                    geofenceData.metadata.breakdown.additional_simulated * 0.1
                  )
                : 0}
              )
            </span>
